...
from model.TSD_model.TSD_blocks import CPSABlock, CTEBlock, LFFNBlock
...
class TSD_T(nn.Module):
    def __init__(self, dim, cte_out_channels, num_encoder_layers=2, expansion_ratio=2):
        ...
        self.encoder_layers = nn.ModuleList(
            [
                TSD_Encoder(dim, num_heads=2, expansion_ratio=expansion_ratio)
...
class TSD_B(nn.Module):
    def __init__(self, dim, cte_out_channels, num_encoder_layers=6, expansion_ratio=4):
        ...
        self.encoder_layers = nn.ModuleList(
            [
                TSD_Encoder(dim, num_heads=4, expansion_ratio=expansion_ratio)
                for _ in range(num_encoder_layers)
            ]
        )
