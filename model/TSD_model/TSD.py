...
class TSD_T(nn.Module):
    def __init__(self, dim, cte_out_channels, num_encoder_layers=2, expansion_ratio=2):
        super(TSD_T, self).__init__()
        self.num_encoder_layers = num_encoder_layers
        self.cte_block = CTEBlock(cte_out_channels, dim)
        self.encoder_layers = nn.ModuleList([
            TSD_Encoder(dim, num_heads=2, expansion_ratio=expansion_ratio) for _ in range(num_encoder_layers)
        ])
...
class TSD_B(nn.Module):
    def __init__(self, dim, cte_out_channels, num_encoder_layers=6, expansion_ratio=2):
        ...
        self.cte_block = CTEBlock(cte_out_channels, dim)
        self.encoder_layers = nn.ModuleList([
            TSD_Encoder(dim, num_heads=4, expansion_ratio=expansion_ratio) for _ in range(num_encoder_layers)
        ])
